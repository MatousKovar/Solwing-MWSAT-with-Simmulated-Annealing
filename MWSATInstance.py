...
class MWSATInstance:
    def __init__(self, filepath, penalty_violation_factor=2):
        ...
        self.num_clauses = 0
        self.weights = [] 
        self.normalized_weights = [] # <--- NEW: Stores 0-1 weights
        self.clauses = [] 
        self.clause_lookup = defaultdict(list)

        self._load_instance()
        self._init_clause_lookup()

        self.total_raw_weight = sum(self.weights)
        ...
        # Penalty is relative to the Normalized scale (0-1)
        # Factor 2 means penalty is 2.0 (stronger than any single variable gain of 1.0)
